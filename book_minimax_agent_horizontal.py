...
AGENT_NAME = os.getenv("AGENT_NAME", "BookedMinimaxHorizontalCutoffV3")
...
BOOST_ALLOW_AT = int(os.getenv("BOOST_ALLOW_AT", "4"))  # do not boost until boosts_remaining <= this value
ENABLE_BOOST = os.getenv("ENABLE_BOOST", "1") != "0"
QUIET = os.getenv("QUIET", "1") != "0"  # if True, suppress all prints/logs from search

# Suppress stdout during internal simulations to avoid game prints from future steps
@contextmanager
def suppress_stdout():
    saved = sys.stdout
    try:
...
def candidate_moves(agent: Agent, board: GameBoard, allow_boost: Callable[[Agent], bool]):
    """Return list of (direction, use_boost_flag).
       - Never include a reverse move.
       - Only include boost when allow_boost(agent) is True AND two cells are clear.
       - Swallows any internal errors to keep search quiet.
    """
    try:
        moves = []
        for d in DIRS:
            ...
            moves.append((d, False))  # include base step
...
def opening_move(game: Game, me_id: int) -> Optional[Tuple[Direction, bool]]:
    ...
    try:
        ...
        # Phase A: move vertically toward opponent's row
...
def clone_game(game: Game) -> Game:
    try:
        ...
        return game  # fallback: return same reference (should be rare)
        ...
        for d_me, b_me in my_actions:
            ...
            for d_op, b_op in op_actions:
                ...
                try:
                    ...
                    continue  # skip illegal transition silently
                if res is not None:
...
def choose_move(game: Game, player_number: int) -> str:
    try:
        ...
        # emergency fallback: keep current direction
        me = game.agent1 if player_number == 1 else game.agent2
        return me.direction.name
